AH_NT = !(missionNamespace getVariable ["AH_NT",false]);
if (AH_NT) then {
	Fragsquad_CQC_ESP = addMissionEventHandler ["Draw3D",
		{
			{
				if ((isPlayer _x) && ((side _x) isEqualTo (side player)) && ((player distance _x) < 250) && (getplayeruid _x != "")) then {
					_pos = getposatl _x;
	...
	["Name Tags On"] spawn CQC_fnc_Notification;
	closeDialog 0;
} else {
	["Name Tags Off"] spawn CQC_fnc_Notification;
	closeDialog 0;
