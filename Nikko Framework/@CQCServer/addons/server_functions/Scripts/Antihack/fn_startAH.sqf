...
try {
	...
	_a pushBack ("
		"+_adminMenuRequest+" = compileFinal (""
			...
			if(_tokenreceived isEqualTo '')exitWith
			{
				...
				_log = _mytime + 'AdminReq tokenreceived is empty (v0.043)';
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
			if(count _array < 2)exitWith
			{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + format['AdminReq bad array: %1 (v0.043)',_array];
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
			if(_clientNetID isEqualTo '')exitWith
			{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + 'AdminReq clientNetID is empty (v0.043)';
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			}; 
			...
			{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + 'AdminReq _ObjFromNetID is Null (v0.043)';
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
			...
			{
				_log = format['%1(%2) | AdminReq - Bad PUID / Token: Token received [%3] belongs to [%4] and not [%2] (v0.043)',_clientName,_clientUID,_tokenreceived,_uid_by_token];
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
			...
			{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + format['AdminReq _playerObj != _ObjFromNetID - %1/2  (v0.043)',_playerObj,_ObjFromNetID];
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
	...
	//-- Load Pre init
	_1 = [] call compile (_a joinString ""); 
	if(isNil {_1}) exitWith {
		['SYSTEMLOG',"Preinit not compiled"] call CQC_fnc_ahLog;
		throw false;
	};
	...
	_b pushBack ("  
		_FNC_AH_KICKFROMSERVER = {
			...
			if(isNil '_input')exitWith{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + 'FNC_AH_KICKLOGSPAWN _this is Nil (v0.043)';
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
			...
			if(isNil '_tokenreceived')exitWith{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + '_tokenreceived _this is Nil (v0.043)';
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
			if(typeName _tokenreceived != 'STRING')exitWith{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + format['_tokenreceived wrong type %1 (v0.043)',typeName _tokenreceived];
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
			...
			if(isNil '_arraysent')exitWith{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + '_arraysent _this is Nil (v0.043)';
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
			if(typeName _arraysent != 'ARRAY')exitWith{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + format['_arraysent wrong type %1 (v0.043)',typeName _arraysent];
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
			...
			if(typeName _netId != 'STRING')exitWith{
				_mytime = call CQC_fnc_serverUpTime;
				_log = _mytime + format['_netId wrong type %1 (v0.043)',typeName _netId];
				['SURVEILLANCELOG',_log] call CQC_fnc_ahLog;
			};
	...
	//-- Load Anti Hack
	_2 = [] call compile (_b joinString ""); 
	if(isNil {_2}) exitWith {
		['SYSTEMLOG',"Antihack not compiled"] call CQC_fnc_ahLog;
		throw false;
	};
	...
	");
	_c pushBack ([CQC_fnc_debugConsole] call CQC_fnc_tooExpression);
	_c pushBack (" 
		[] spawn {
		...
		true
	");
	uiNamespace setVariable [_adminPayload,compileFinal (_c joinString "")];
	_3 = (isNil {_3}|| {(uiNamespace getVariable _adminPayload) isNotEqualType {}});
	if(_3) exitWith {
		['SYSTEMLOG',"Adminmenu not compiled"] call CQC_fnc_ahLog;
		throw false;
	};
