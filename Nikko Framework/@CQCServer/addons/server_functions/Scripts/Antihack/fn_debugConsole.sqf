...
fnc_lbDebugChanged = {
	...
	((findDisplay -1341) displayCtrl 1) ctrlSetText (['CQC_SAVED_FUNCTIONS',[]] call fnc_getPNS select _index);
};
fnc_fillDebugListBox = {
	...
	_var = ['CQC_SAVED_FUNCTIONS',[]] call fnc_getPNS;
...
fnc_debug_loadbtn = {
	...
	{
		_varvalue = [format['CQC_SAVED_FUNCTION_%1',_varname],''] call fnc_getPNS;
...
disableSerialization;
if(isNull findDisplay -1341)then{createdialog 'CQC_EDITBOX2';'AdminConsole' call fnc_adminLog;};
...
_ctrl ctrlCommit 0;
_ctrl ctrlSetEventHandler['ButtonClick','[call fnc_getMainText] call admin_d0;systemChat ''<CQC.de> RAN CODE ON: GLOBAL (SERVER AND ALL CLIENTS)'';true'];
...
_ctrl ctrlCommit 0;
_ctrl ctrlSetEventHandler['ButtonClick','[''if(!isServer)then{''+(call fnc_getMainText)+''};''] call admin_d0;systemChat ''<CQC.de> RAN CODE ON: ALL BUT SERVER'';true'];
...
_ctrl ctrlCommit 0;
_ctrl ctrlSetEventHandler['ButtonClick','[call fnc_getMainText] call admin_d0_server;systemChat ''<CQC.de> RAN CODE ON: SERVER'';true'];
...
_ctrl ctrlCommit 0;
_ctrl ctrlSetEventHandler['ButtonClick','if(isPlayer SELECTED_TARGET_PLAYER)then{[call fnc_getMainText,SELECTED_TARGET_PLAYER] call admin_d0_target;systemChat format[''<CQC.de> RAN CODE ON: %1'',name SELECTED_TARGET_PLAYER];};true'];
};
...
_ctrl ctrlCommit 0;
_ctrl ctrlSetEventHandler['ButtonClick','(call fnc_getMainText) call fnc_admin_cc;systemChat ''<CQC.de> RAN CODE ON: LOCAL'';true'];
...
_ctrl ctrlCommit 0;
_ctrl ctrlSetEventHandler['ButtonClick','if(!isNull cameraOn)then{[call fnc_getMainText,cameraOn] call admin_d0_target;systemChat format[''<CQC.de> RAN CODE ON: %1'',name cameraOn];};true'];
};
...
_ctrl ctrlSetEventHandler['ButtonClick','
	...
	systemChat format[''<CQC.de console> %1 / %2'',last_selected_run_id,_max];
true
'];
...
_oldTxt = ['CQC_show_function_000',''] call fnc_getPNS;
...
_ctrl ctrlSetEventHandler['ButtonClick','
	...
	[''CQC_show_function_000'',_ctrltext] call fnc_setPNS;
...
_ctrlid = _ctrlid + 1;
_ctrl = [_display,'RSCEdit',_ctrlid] call fnc_createctrl;
_ctrl ctrlSetText (['CQC_show_variable_000','READ FROM VARIABLE'] call fnc_getPNS);
_ctrl ctrlSetPosition [1,_buttonYpos,0.25,_buttonheight];
_ctrl ctrlSetBackgroundColor [0,0,0,0.6];
_ctrl ctrlCommit 0;
...
_ctrl ctrlSetEventHandler['ButtonClick','
	...
	[''CQC_show_variable_000'',_ctrltext] call fnc_setPNS;
...
_ctrl ctrlSetEventHandler['ButtonClick','
	...
	else
	{
		...
		[''CQC_SAVED_FUNCTIONS'',_var] call fnc_setPNS;
		[format[''CQC_SAVED_FUNCTION_%1'',_varname],call fnc_getMainText] call fnc_setPNS;
	...
	else
	{
		...
		else
		{
			if(last_delete_call isEqualTo _varname)then
			{
				...
				[''CQC_SAVED_FUNCTIONS'',_var] call fnc_setPNS;
				[format[''CQC_SAVED_FUNCTION_%1'',_varname],nil] call fnc_setPNS;
